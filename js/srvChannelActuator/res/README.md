...
- #_ValueConfirm - флаг, true => поле #_Value обновляется только при подтверждении от прокси-служб, false => автоматически;
- #_StateChName - имя сенсорного канала, значение которого транслируется в SetValue(x) актуатора.
