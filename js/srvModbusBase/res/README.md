...
### Описание
Модуль реализует базовый функционал служб, использующих протокол modbus. Службы-клиенты серверного фреймворка используют объект данного класса для установки соединения с сервером по указанному протоколу (RTU, TCP/IP, RTU over TCP/IP), а также стандартизирует набор команд протокла modbus и исключает Служба предназначена для взаимодействия с логическими контроллерами по протоколу ModBus-TCP:
